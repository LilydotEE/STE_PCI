...
import numpy as np
from model.flowmodel import PointConvBidirection
from pointnet2 import pointnet2_utils
from model.pointconv_util import knn_point, FrameWarping, MLP
from model.dgcnn import FeatureWarping
...
class Net(nn.Module):
    ...
    def forward(self, points1, points2, color1, color2, t):
        ...
        # f = FrameSampling(f1, f2, t)
        # 按时序比例随机采样得到f B N C
        f = torch.cat((f1, f2), dim=2)
        #  N 2C
        # f_temp = f1.transpose(1,2).contiguous()
        ...
        f = self.fusion(f)
        f = f.squeeze(-1)
        base_f = f.transpose(1, 2).contiguous()
        # B C N

        f1 = f1.transpose(1, 2).contiguous()
        f1 = f1.unsqueeze(-1)
        f2 = f2.transpose(1, 2).contiguous()
        f2 = f2.unsqueeze(-1)
        # MLP input B C N 1
        w1 = self.mlp_layer(f1)
        # 过一系列MLP，得到 B 1 N 1
        w1 = w1.squeeze(1)
        # 得到 B N 1
        w2 = self.mlp_layer(f2)
        w2 = w2.squeeze(1)
        # 得到 B N 1
        ...
        out1 = torch.mul(w1, base_f)
        out2 = torch.mul(w2, base_f)
        # B N C
