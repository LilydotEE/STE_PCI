...
def parse_args():
    ...
    parser.add_argument('--train_root', type=str, default='/home/gxz/桌面/DHB-dataset/train')
    parser.add_argument('--save_dir', type=str, default='model_trained/')
    ...
    parser.add_argument('--model_for_dhb', type=str, default='model_trained/model_for_MITAMA.pth')
...
def train_inter(args):
    ...
    train_loader = DataLoader(train_dataset,
                              ...
                              pin_memory=True,
                              drop_last=False)
    net = Net(k_warping = args.k_warping, dgk = args.dgk, fpk = args.fpk, down_num = args.down_num, npoints = args.npoints, freeze = args.freeze, use_bn=args.use_bn).cuda()
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % 100 == 0:
            torch.save(net.state_dict(), args.mid_save_dir + 'model_ft_' + str(epoch) + args.save_name + '.pth')

        if total_loss < best_loss:
            torch.save(net.state_dict(), args.save_dir + 'model_ft' + args.save_name + '.pth')
